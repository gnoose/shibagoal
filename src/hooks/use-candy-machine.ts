...
import {MintRequest, TokenResponse} from '../core/types/token';
  ...
  const onMintMultiple = async (quantity: number) => {
    ...
    try {
      ...
      if (wallet.connected && candyMachine?.program && wallet.publicKey) {
        ...
        const res = await mintMultipleToken(
          candyMachine,
          config,
          wallet.publicKey,
          treasury,
        ...
        const signedTransactions: any = res[0];
        tokenList = res[1];
      ...
      // update minted token before the reveal time
      for (let index = 0; index < tokenList.length; index++) {
        // let tokenPubkey = "AVN2AbqvzqwwgatCd1btXX7u99cWDkCLQqZ6VKvD1WET";//tokenList[index].publicKey.toBase58();
        let tokenPubkey = tokenList[index].publicKey.toBase58();
        console.log('tokenPubkey = ', tokenPubkey);
        const body: MintRequest = {
          owner: ownerWallet.toBase58(),
          token: tokenPubkey,
        };
        TokenService.mint(body).then((res: TokenResponse) => {
          console.log('mint info = ', res);
        }).catch(() => {
          console.log('mint token launched error');
        }).finally(() => {
          console.log('mint token finally finsihed');
        });
      }
      // end updating minted token
    }
  };
